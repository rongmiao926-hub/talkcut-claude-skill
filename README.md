...
# 🎬 第 2 步：开始剪辑（最关键的一步）
/videocut:cut /path/to/your/video.mp4
...
# 📽️ 可选：添加字幕（让你的视频更专业）
/videocut:subtitle /path/to/your/video.mp4
```
...
你可以通过以下 prompt 让 AI 帮你找到文件：

1. **查看字典位置**：
   ```
   用户: videocut 字典文件在哪里？
   AI: 字典文件在 ~/.claude/skills/videocut/字幕/词典.txt
   ```

2. **查看用户习惯目录**：
   ```
   用户: videocut 用户习惯文件在哪里？
   AI: 用户习惯文件在 ~/.claude/skills/videocut/剪口播/用户习惯/
   ```

3. **查看所有配置文件**：
   ```
   用户: videocut 的配置文件有哪些？
   AI: 主要配置文件包括：
   - ~/.claude/skills/videocut/字幕/词典.txt（自定义词典）
   - ~/.claude/skills/videocut/剪口播/用户习惯/（审核规则）
   - ~/.claude/skills/videocut/剪口播/show-notes.md（文案生成规则）
   ```
...
1. **让 AI 打开文件**：
   ```
   用户: 打开 ~/.claude/skills/videocut/字幕/词典.txt
   ```
...
**日常使用**：
- 只需要在 Claude Code 里打字就行，比如：
  ```
  /videocut:cut 我的视频.mp4
  ```
- 不需要记任何命令，也不需要打开终端
